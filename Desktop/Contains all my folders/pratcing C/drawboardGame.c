...
/*
* this is a drawboard game built in c
*/
...
// Function to deal the first five cardsvoid getFirstHand(int cardRank[], int cardSuit[])
void getFirstHand(int cardRank[], int cardSuit[])
{
    int i,j;
    int cardDup;
...
} while (cardDup == 1);ranksinHand[finalRank[i]]++;
suitsinHand[finalSuit[i]]++;
}
}
}